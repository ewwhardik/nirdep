...
const full = () => tree({
  ...
  [`${VECTORS}/collect/values.json`]: vector(147),
  ...
  [`${DRIVERS}/collect.test.mjs`]: driver('collect'),
});
...
test('the modules and the packages they replace come off the rule catalogue', () => {
  assert.deepEqual(MODULES.map((one) => one.name), ['colour', 'semver', 'glob', 'collect', 'args']);
...
test('a case count is the length of an array in a file, not a number somebody typed', () => {
  ...
  assert.equal(plan.totals.cases, 74 + 94 + 432 + 300 + 147);
  assert.equal(plan.totals.vectors, 6, 'the lex vectors belong to another layer');
...
test('this repository is its own fixture, and the numbers are real', () => {
  ...
  assert.equal(plan.totals.packages, 11);
